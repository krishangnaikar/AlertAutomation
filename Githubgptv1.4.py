...
def is_article(url):
    ...
    # Compose prompt
    prompt = f"Is the following link an article?\n{url}\n\nAnswer:"
...
def Find_Links(subject):
    ...
    if 'google alert' in subject.lower():
        ...
        article = []
        for link in links:
            article.append(is_article(link))
        df['Is Article'] = pd.Series(article)
        # Note: Make sure to handle the remaining code accordingly after the loop

        # Save the DataFrame to an Excel file
        df = df.reindex(['Date', 'Subject', 'Formatted Urls', 'Is Article'], axis=1)
        excel_file = 'links2.xlsx'
...
# The Alerts you want to search for
